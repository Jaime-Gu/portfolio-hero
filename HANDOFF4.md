...
30. **Works 改名 Academy**：标题 `Works ⌘` → `Academy 🎓`
31. **二次上线 3d31591**（用户发话）：Projects 上移 + Academy + 右区整幅模糊 + 双主题调暗推 GitHub，Pages 验证全 200
32. **Vercel 上线**：`npx vercel deploy --prod --yes`（CLI 59.15.1，设备码登录流程在浏览器完成授权），生产地址 **https://portfolio-hero-flame.vercel.app**，验证全 200。**CLI 手动包不会随 git push 自动更新，要自动部署需 `vercel git connect`**
35. **Vercel 项目改名 jaime-gu**：`vercel project rename portfolio-hero jaime-gu` + `vercel alias set <部署URL> jaime-gu.vercel.app`。坑：改名后默认域名不自动指向部署（要手动 alias）；新域名 302 跳 SSO 登录——项目 `ssoProtection.deploymentType = "all_except_custom_domains"`，API `PATCH /v9/projects/jaime-gu {"ssoProtection": null}` 关闭（**"none" 不是合法 deploymentType，必须置 null**；token 在 `~/Library/Application Support/com.vercel.cli/auth.json`）。**正式地址 https://jaime-gu.vercel.app（200 全绿）**
36. **Academy 重构（两校校徽版，本地未上线）**：用户提供简历 PDF + UCL/浙大校徽，点名"写入 Academy，实习部分忽略，全部英文，不写成绩/职务，写有意义的"。简历教育信息：浙大 2023.09–2027.06 西班牙语+社会学辅修；UCL 2026.01–2026.06 交换生。两 work-group 替换为 `.edu-group`：校徽玻璃小卡（96px 圆角卡，深/浅两主题）+ 校名 + 项目/时间 + 一句 note。校徽来源：UCL PNG（Wikimedia Commons `File:Ucl_logo.png`）、ZJU 校徽 SVG（`upload.wikimedia.org/wikipedia/zh/1/16/Zhejiang_University_Logo.svg`，fill #004EA2 正品蓝圆校徽；commons 搜索不到时按文件名 md5 探 `/wikipedia/{lang}/{h}/{h2}/{filename}` 路径）
37. **Academy 二轮迭代**：① meta 改 `Exchange, Arts and Science · Jan – Jun 2026`（"Student" 删、"(expected)" 删）；② note 加长加具体（浙大：四年西语+社会学辅修从拉美文学到社会研究方法；UCL：Arts and Science 交换学期人文社科课程+跨文化设计技术课堂）；③ 校徽卡 96px 方卡改长方形（72px 高，宽自适应）；④ 深/浅双版 logo 切换（`.logo-light`/`.logo-dark` 按 data-theme 显隐）：UCL 官方双色（用户 Downloads `brand-VI-logo-horizontal-fullcolour[-white].png.webp`，浅色平色深紫、深色白版+亮紫门廊，白版深紫底转透明阈值 65 保门廊）；ZJU 组合标（用户 Downloads `images.png`：校徽+浙江大学毛体字标+ZHEJIANG UNIVERSITY，裁白边 411×116，蓝版浅色用、反白版深色用）。**composer 图片附件只有内联预览不落盘——要文件须让用户存到 Downloads/assets 或 Wikimedia 自取**
38. **阿尔卡拉大学加入 + 三校卡片统一尺寸**：新增第三条 `.edu-group`：`University of Alcalá` + `Summer School · Jun – Jul 2024` + 简短 note（"Summer school in Alcalá, Spain — a month of language and culture immersion."）。logo 用用户 Downloads `vA_azul/blanco_LogoPrincipal.png`（背景本透明，裁边即用）。三校液态玻璃校徽卡统一 `200×72px`（用户点名"大小统一，不要有的大有的小"）
39. **Academy 三轮迭代（裸 logo + 玻璃文本卡）**：① 布局改"左 logo 裸放（无背景无边框无阴影，列宽 200px）+ 右正文液态玻璃卡（var(--bg)/border/内发光三件套，flex:1 卡宽统一，间距 1.5rem 统一，高度随内容可不同）"；② 深色 edu-meta 时间信息改 `var(--fg)`（JAIME GU 同色，不用丑橙）；③ UCL 浅色 logo 抠图：原图自带 2-3px 灰紫细框（lum~130 sat 0.19-0.35，亮度/饱和度阈值都清不掉），最终裁边 4px 去除（alpha bbox 收紧到 76,120,631,276 内容区）；④ UCL logo 放大 40%（`img.ucl { max-height: 100px }`）；⑤ ZJU logo 白框：Downloads 的 images.png 背景是不透明白（裁边只裁了尺寸没抠透明），泛洪近白（lum>230）转透明
40. **edu 卡换 blog 玻璃配方 + Blog 区复制 + UCL 白版框线清除**：① edu-card 改 blog 卡同款液态玻璃（light：`linear-gradient(135deg, rgba(255,255,255,0.50), rgba(255,255,255,0.15)), rgba(255,255,255,0.22)` + `blur(24px) saturate(1.4)` + 1px rgba(255,255,255,0.7) 亮描边 + 内高光/软阴影三件套；dark：rgba(150,175,205,0.28) 底 + 对应阴影；radius 20px 36px 20px 36px）；② Blog 区整段复制到 Academy 与 Blog 之间，靠上的一份标题改 `Photograph 📷`（图标换相机）；③ UCL 白版 logo 也带 1px 紫框线（lum 83 超阈值），裁边 2px 清除
41. **Education 改名 + 社媒链接 + 领英归位**：① 区块标题 `Academy 🎓` → `Education 🎓`；② 写入社媒：GitHub→`github.com/Jaime-Gu`、X→`x.com/jaimfees?s=11`、Instagram→`instagram.com/jaimeeegu/`、LinkedIn→`linkedin.com/in/zhongyu-gu-98ab243a8/`（新增按钮，simple-icons "in" SVG）、邮箱→`jaime.guuuu@gmail.com`（CONTACT 列 + footer "Say hi" 同步）；③ 领英按用户点名从 SOCIALS 列移到 CONTACT 列（与邮箱、微信同列）
42. **Photograph 改双排胶卷循环滚动**：删掉 Blog 复制卡，改双排胶卷。素材 `~/Desktop/photograph`（26 张去重，横竖混合）压缩到高 800px q80 入 `assets/photos/`（a01-a14 上排、b01-b12 下排）。实现：`.filmstrip` 两 `.film-row`，每行 `.film-track` 内含两份相同 `.film-set`（×2 无缝）；JS 每帧 `off = (scrollY×0.35) % setW`，上排 `translateX(-off)` 左移、下排 `translateX(off-setW)` 右移——随页面上下滑左右滑、取模无缝循环，不依赖悬停。照片统一高 320px（横竖同高）、圆角 14px、液态玻璃光泽（`.film-photo::after` 亮描边 + 顶部受光渐变 + 软阴影，深色另写）。区域高度 ~700px（用户点名扩到 ~2×）
43. **胶卷迭代**：① 误加 `1790.JPG`（足球拥抱照）删除（b01 两张 span + 文件）；② 速度 0.35→1.0；③ 照片高度 320→256px（-20%，一屏更多照片）；④ 间距统一 20px（track/set/换行点同距）
33. **滚动跟手优化（20260910zc，视觉/交互零改动）**：用户报"不跟手、上下滑动不丝滑"。五处优化：① fx-gl 渲染 60fps 封顶（120Hz 屏 GPU 预算减半，慢动柔雾无感）；② 模糊中间目标 B 半分辨率（模糊 pass 成本 ÷4；**uv 映射要分 uRes 目标尺寸/uSrcRes 源尺寸**，半分辨率下同一 texel 公式会只采左下 1/4）；③ 拖尾核 7×3→5×3 taps（权重重归一，-29% 成本）；④ `.blog-stream`/`.contact-panel` 加 `content-visibility: auto` + `contain-intrinsic-size`（折叠区以下跳过渲染，防滚动条跳动）；⑤ `.fan-card` `will-change: transform`（合成层提升）。像素级对比优化前 mean diff 4.84/255（仅动画相位差，结构无变）
34. **深色右缘亮条宽度不均修复（20260910zd，本地未上线）**：用户圈图"白色发光条宽度变化不均匀"。像素剖面定位三处：顶角区（角弧度+b1）、中段 b8 凸起、底部渐窄。修复：① 深色边缘光加力压过云团调制（`inset -2px 0 6px` 0.38→0.60、`inset -8px 0 20px` 0.16→0.30）；② **b8 凸起主因是它的中心压在边界羽化带（清晰态 aSharp 主导，阻尼 aBlur 无效——两版像素剖面对照实验证明）**——b8 左移出羽化带 56%→52%（fx-gl POS 与 CSS 同步）；③ 深色 aBlur 近缘渐暗（`aBlur *= mix(1.0, 0.55+0.45×smoothstep(uSbw-40, uSbw+200, css.x), uTheme)`，浅色不生效）。修后剖面中段 35-40 均匀（原 24-44）
